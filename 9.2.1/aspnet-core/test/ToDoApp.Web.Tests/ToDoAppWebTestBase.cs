...
{
    public abstract class ToDoAppWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ToDoAppWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ToDoAppWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ToDoAppDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ToDoAppDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ToDoAppDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ToDoAppDbContext, Task<T>> func)
        {
            T result;
